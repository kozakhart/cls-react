...
def start_search(record_date_valid_by, semester_date_valid_by):
    ...
    for student in record_response['response']['data']:
        if student['fieldData']['EntryDate'] >= semester_date_valid_by and student['fieldData']['CertificateStatus'] != "Email" and student['fieldData']['CertificateStatus'] != "Awarded":
            ...
            if (opi != None or opic!= None) and wpt != None:
                student_data = {"recordId": recordId, "firstname": firstname, "lastname": lastname, "byuid": byuid, "netid":netid, "language": {abbre_language: wpt[2]}, "opiScore": opi[0] if opi != None else 'None',
