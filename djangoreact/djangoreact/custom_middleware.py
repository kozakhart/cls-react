...
class TokenVerificationMiddleware:
    ...
    def __call__(self, request):
        if request.path.startswith('/admin/') and not request.path.startswith('/admin/login/'):
            if 'token' not in request.COOKIES:
                return HttpResponseRedirect(f'/admin/login/')  # Redirect to admin login with current path as next
            ...
            if token:
                try:
                    ...
                    auth_token = AuthToken.objects.get(token_key=token, user__username=username)
                    if auth_token and auth_token.user.username == username and auth_token.expiry > timezone.now():
                        return self.get_response(request)  # Continue to the requested view for /admin/
                    else:
                        return HttpResponseRedirect(f'/admin/login/?next={request.path}')  # Redirect if token is invalid for /admin/
                except AuthToken.DoesNotExist:
                    return HttpResponseRedirect(f'/admin/login/?next={request.path}')  # Redirect if token does not exist for /admin/
        
        # Continue to the requested view for other URLs
        return self.get_response(request)